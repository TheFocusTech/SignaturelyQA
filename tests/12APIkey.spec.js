...
test.describe('API key', () => {
    test('TC_12_48_01 | Verify user can copy API key created by the "Create API" button on the right', async ({
        ...
        await description('To verify user can copy API key created by the "Create API" button on the right');
        ...
        await step('Verify the toast message API key has been successfully copied to clipboard', async () => {
    ...
    test('TC_12_48_02 | Verify user can copy API key created by the "Create API" button in Table', async ({
        ...
        await settingsAPIPage.fillBillingDetailsField(clipboardApiKeyValue);
            ...
            await step('Verify the pasted API key matches the one created with "Create API" button in Table', async () => {
            await expect(settingsAPIPage.billingDetailsTextField).toHaveText(clipboardApiKeyValue);
        });
    });
    ...
    API_PLANS.forEach((apiPlan) => {
        test(`TC_12_49_01 | Verife user can purchase ${apiPlan} API plan`, async ({
            ...
            await description('To verify Business user can successfully purchase an API plan');
            ...
            await step('Verify the toast with "Api plan have been upgraded" appears', async () => {
                await expect(settingsAPIPage.toast.toastBody).toHaveText(TOAST_MESSAGE.apiPlanUpgraded);
            });
            await step('Verify the selected plan is marked as Current plan', async () => {
    ...
    test('TC_12_50_01 | Verify user can Upgrade/Downgrade API subscription', async ({
        ...
        for (let i = 1; i < API_PLANS.length; i++) {
            ...
            await step('Verify the selected API plan is marked as Current plan', async () => {
                await expect(settingsAPIPage.apiPlansList.nth(i)).toContainText(currentPlan);
            });
            await step('Verify the current plan is upgraded to higher one', async () => {
        ...
        for (let i = API_PLANS.length - 2; i >= 0; i--) {
            ...
            await step('Verify the Title of Downgrade Modal Window has the text "Downgrade to selected Plan"', async () => {
