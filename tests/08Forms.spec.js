...
import { FORM_NAME, OPTIONAL_MESSAGE_TEXT, FORM_NAME_EDIT, OPTIONAL_MESSAGE_TEXT_EDIT, TOASTER_MESSAGE } from '../testData.js'
...
test.describe('Forms', () => {
  ...
  test('TC_08_33_01 | "Forms" - verify that user can edit form.', async ({ page, loginBusinessUser, createForm }) => {
    ...
    const formsCheckBoxes = await formRequestsPage.locators.getCheckBoxOfForm();
    const formsCount = await formsCheckBoxes.count();
    const timeout = Date.now() + 60000;

    for (let i = 0; i < formsCount && Date.now() < timeout; i++) {
      await formRequestsPage.clickCheckBoxOfForm(i);
